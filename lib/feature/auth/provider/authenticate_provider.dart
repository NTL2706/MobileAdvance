...
class AuthenticateProvider extends ChangeNotifier{
  ...
  AuthenticateProvider(){
    if (authenRepository.role != ""){
      state = AuthState(result: AuthResult.success, isLoading: false, userName: authenRepository.username);
    }
  ...
  Future<void> signInWithPassword({
    ...
    notifyListeners();
    final response = await authenRepository.signInWithPassword(email: email, password: password, role: role);
   
    if (response['result'] == AuthResult.success){
      await getUserInf();
    }
    state = AuthState(result: response['result'], isLoading: false, userName: authenRepository.username, message: response['message']);
    notifyListeners();
  }
  ...
  Future<void> signUpWithPassword({
    ...
    state = state.copiedWithIsLoading(true);
    notifyListeners();
    final response = await authenRepository.signUpWithPassword(email: email, password: password, fullname: fullname, role: role);
    state = AuthState(result: response['result'], isLoading: false, userName: authenRepository.username,message: response['message']);
    notifyListeners();
  }
